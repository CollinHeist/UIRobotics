...
int HandleInput()
{
	...
	int rx4_flag = getcU4(&Char);  // Poll for UART character received
	if (rx4_flag) CharReceived = 1;

	// If a char was received
	if (CharReceived)
	{
		// Adding this character to the approp. position in the object
		GamepadInputManager.m_InputString[GamepadInputManager.m_Offset] = Char;
