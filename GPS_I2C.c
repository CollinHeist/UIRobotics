...
/* ------------------------------ HeadingAngle ------------------------------
  @ Summary
     Returns a Compass heading (ie N is 0, E is 90, W is 270) from one coordinate
 *   to another
  @ Parameters
     @ param1 : x coord of current position
 *   @ param2 : y coord of current position
 *   @ param3 : x coord of destination position
 *   @ param4 : y coord of destination position
  @ Returns
     float : a 0 to 359 degree heading
  ---------------------------------------------------------------------------- */
float HeadingAngle(float x1, float y1, float x2, float y2){
    float heading = 0;
    float xt = x2 - x1;
    float yt = y2 - y1;
    if(xt < 0){     //Areas II & III
        heading = 270 - (45 / atan(1)) * atan(yt/xt);
    }
    else{           //Areas I & IV
        heading = 90 - (45 / atan(1)) * atan(yt/xt);
    }
    return heading;
}
