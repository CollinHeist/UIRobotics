...
extern char DMABuffer[];	// Master DMA UART Rx Buffer
...
int initializeGamepad(void) {
	...
	for (i = 0; i < GAMEPAD_INPUT_STRING_SIZE; i++) {
...
int HandleInput(void) {
	...
	if (CharReceived) {
		...
		for (i; DMABuffer[i] != 0; i++) {
			GamepadInputManager.m_InputString[GamepadInputManager.m_Offset] = DMABuffer[i];
